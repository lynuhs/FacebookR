...
fb_check_existing_token <- function(token = FacebookAuth$public_fields$token){
  ...
  cache_path <- is.different(token$cache_path, "FacebookR.oauth_cache")

  if(!is.null(token$app)){
    app_id     <- is.different(token$app$key, "FacebookR.client_id")
    app_secret <- is.different(token$app$secret, "FacebookR.client_secret")
